...
def snyk_open_source(repo_path):
    ...
    if results.stderr != "":
        error(f"Snyk open source encountered an error when analyzing {repo_path}: {results.stderr}")
    try:
    ...
    # extract only the "vulnerabilities" and "summary" fields

    return_value = {
        "vulnerabilities": snyk_output.get("vulnerabilities", ""),
        "summary": snyk_output.get("summary", ""),
        "uniqueCount": snyk_output.get("uniqueCount", ""),
    }
...
def snyk_container(container_url):
    ...
    results = subprocess.run(
        ["snyk", "container", "test", container_url, "--json"],
        capture_output=True,
        text=True,
    )
    if results.stderr != "":
...
def get_git_signed_commit_count(repo_path):
    ...
    # Get the number of signed commits
    signed_commits = subprocess.check_output(
        "git log --show-signature | grep 'gpg: Good signature' -B3 | grep 'commit' | awk '{print $2}' | wc -l",
        shell=True,
    )
...
def snyk_monitor(repo_path):
    ...
    if results.stderr != "":
        error(f"Snyk monitor failed with error: {results.stderr}")

    try:
        snyk_output = json.loads(results.stdout)
    except:
        error(f"Snyk monitor json output intepretation failed: {results.stdout}")
        return {}
...
def run_analysis(repo_path, language, parsed_data):
    ...
    debug(f"Getting percentage of signed commits for {repo_path}")
    parsed_data["git_commit_signatures_count"] = int(
        get_git_signed_commit_count(repo_path)
    )

    # Get percentage of signed commits
    if int(parsed_data["git_commit_count"]) > 0:
        parsed_data["git_commit_signatures_percentage"] = (
            int(parsed_data["git_commit_signatures_count"])
            / int(parsed_data["git_commit_count"])
            * 100
        )
    else:
    ...
    debug(
        f"{repo_path} has {parsed_data['git_commit_signatures_count']}/{parsed_data['git_commit_count']} signed commits"
    )
